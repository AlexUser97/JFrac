...
"""
function projection_from_ribbon(ribbon_elts::Vector{Int}, channel_elts::Vector{Int}, mesh::CartesianMesh, sgnd_dist::Vector{Float64})::Vector{Float64, NaN}
...
"""
function find_angle(elt_ribbon::Vector{Int}, elt_tip::Vector{Int}, zr_vrtx_tip)
def find_angle(elt_ribbon, elt_tip, zr_vrtx_tip, a_tip, b_tip, c_tip, x_lft, y_lft, x_rgt, y_rgt, neig_lft,
    ...
    closest_tip_cell = np.zeros((len(elt_ribbon),), dtype=np.int32)
...
Returns the fracture toughness at the zero-vertex location. Handles both heterogeneous and anisotropic toughness.

# Arguments
- `elts::Vector{Int}`: list of element indices
- `mesh::CartesianMesh`: mesh with `Connectivity` and `VertexCoor`
- `mat_prop::MaterialProperties`: material properties with `K1c`, `K1cFunc`, `anisotropic_K1c`
- `alpha::Vector{Float64}`: angle of the front segment (radians)
- `l::Vector{Float64}`: distance from vertex to front
- `zero_vrtx::Vector{Int}`: which vertex is the zero-vertex (0, 1, 2, 3)

# Returns
- `K1c::Vector{Float64}`: toughness at zero-vertex points
"""
function get_toughness_from_zeroVertex(
    elts::Matrix{Int},
    mesh::CartesianMesh,
    mat_prop::MaterialProperties,
    alpha::Matrix{Float64},
    l::Matrix{Float64},
    zero_vrtx::Matrix{Int}
)::Vector{Float64}

    # Case 1: no spatial variation — return constant or per-element K1c
    if mat_prop.K1cFunc === nothing
        return mat_prop.K1c[elts]
    end

    # Case 2: anisotropic toughness — depends on angle alpha
    if mat_prop.anisotropic_K1c
        return mat_prop.K1cFunc.(alpha)  # поэлементно, если K1cFunc скалярная
    end

    # Case 3: heterogeneous toughness — depends on spatial coordinates (x, y)
    N = length(elts)
    x = zeros(N)
    y = zeros(N)
    ...
    @inbounds for i in 1:N
        e = elts[i]           # элемент
        v_idx = zero_vrtx[i] + 1  # ← 0 → 1, т.к. Julia индексирует с 1
        conn = mesh.Connectivity[e, v_idx]  # номер вершины
        ...
        if zero_vrtx[i] == 0
            x[i] = mesh.VertexCoor[conn, 1] + ℓ * cosα
            y[i] = mesh.VertexCoor[conn, 2] + ℓ * sinα
    ...
    # Evaluate spatially varying toughness function K1cFunc(x, y)
    K1c = similar(x)
    @inbounds for i in 1:N
        K1c[i] = mat_prop.K1cFunc(x[i], y[i])
    end
...
Arguments:
    alpha:Matrix{Float64}             -- the angle inscribed by the perpendiculars drawn on the front from the \
                                         ribbon cell centers.
    Cij:Matrix{Float64}               -- the TI stiffness matrix in the canonical basis

Returns:
    E':Matrix{Float64}               -- plain strain TI elastic modulus.
"""
function TI_plain_strain_modulus(alpha::Matrix{Float64}, Cij::Matrix{Float64})::Matrix{Float64}
    ...
    alphag = (C11 * (C11 - C12) * cos.(alpha) .^ 4  + (C11 * C13 - C12 * (C13 + 2 * C44)) .* (cos.(alpha)
     .* sin.(alpha)) .^ 2 - (C13^2 - C11 * C33 + 2 * C13 * C44) * sin.(alpha) .^ 4 + C11 * C44 *
      sin.(2 * alpha) .^ 2) ./ (C11 * (C11 - C12) * cos.(alpha) .^ 2 + 2 * C11 * C44 * sin.(alpha) .^ 2)
