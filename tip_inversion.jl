...
def TipAsym_Universal_1stOrder_Res(dist, *args):
    ...
    if Cbar == 0:
        return TipAsym_MK_deltaC_Res(dist, *args[:-1])
...
def FindBracket_dist(w, Kprime, Eprime, fluidProp, Cprime, DistLstTS, dt, mesh, ResFunc, simProp, muPrime):
    """ 
    ...
    if fluidProp.rheology == "Newtonian" or sum(Cprime) == 0:
        ...
        for i in range(0, len(w)):
            ...
            b[i] = fsolve(Vm_residual, (w[i] * Eprime[i] / Kprime[i])**2, args=TipAsmptargs) #doesn't require constant value of viscosity
        
        
    for i in range(0, len(w)):

        _muPrime = muPrime[i] if isinstance(muPrime, np.ndarray) else muPrime
        TipAsmptargs = (w[i], Kprime[i], Eprime[i], fluidProp, Cprime[i], -DistLstTS[i], dt, _muPrime)
        Res_a = ResFunc(a[i], *TipAsmptargs) # constant value of the viscosity is lacing in any ResFunc
...
def TipAsymInversion(w, frac, matProp, fluidProp, simParmtrs, dt=None, Kprime_k=None, Eprime_k=None, perfNode=None):
    ...
    _muPrime = fluidProp.muPrime[frac.EltRibbon[moving]] if isinstance(fluidProp.muPrime, np.ndarray) else fluidProp.muPrime
    ...
    for i in range(0, len(moving)):
        _muPrime = fluidProp.muPrime[frac.EltRibbon[moving[i]]] if isinstance(fluidProp.muPrime, np.ndarray) else fluidProp.muPrime
