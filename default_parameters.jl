...
max_solver_itrs = 100                   # maximum iterations for the elastohydrodynamic solver.
max_proj_Itrs = 10                      # maximum projection iterations.

# time and time stepping
tmStp_prefactor = 0.5                   # time step prefactor(pf) to determine the time step(dt = pf*min(dx, dy)/max(v).
req_sol_at = None                       # times at which the solution is required.
final_time = None                       # time to stop the propagation.
maximum_steps = 5000                    # maximum time steps.
timeStep_limit = None                   # limit for the time step.
fixed_time_step = None                  # constant time step.

# time step re-attempt
max_reattemps = 20                       # maximum reattempts in case of time step failure.
reattempt_factor = 0.5                  # the factor by which time step is reduced on reattempts.
...
aspect_ratio = None                     # fracture aspect ratio to calculate the analytical solution for TI case.                # fracture aspect ratio to calculate the analytical solution for TI case.

# roughness parameters
roughness_model = None
roughness_sigma = None

# real time from experiment
reference_time = None