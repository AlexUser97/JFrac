...
corr_functions = {
    'Amadei_Illangasekare': lambda sigma, dm: dm * (1 / (1 + 0.6 * (sigma/dm) ** 1.2))**(1/3),
    'Patir_Cheng': lambda sigma, dm: dm * np.cbrt(1 - 0.9 * np.exp(-0.56 * dm / sigma)),
    'Quadros': lambda sigma, dm: dm * np.cbrt(1 / (1 + 20.5 * (sigma/2/dm) ** 1.5))
}
...
def finiteDiff_operator_laminar(w, EltCrack, muPrime, Mesh, InCrack, neiInCrack, simProp):
    ...
    Args:
        ...
        muPrime (ndarray | np.ndarray):      -- the scaled local viscosity of the injected fluid (12 * viscosity).
    ...
    """
    if simProp.roughness_model is not None and simProp.roughness_sigma is not None:
        corr_function = corr_functions[simProp.roughness_model]
        def roughness_func(sigma, dm):
            return corr_function(sigma, dm)
    ...
    if simProp.roughness_model and simProp.roughness_sigma:
        wLftEdge, wRgtEdge, wBtmEdge, wTopEdge = list(map(lambda dm: roughness_func(dm = dm, sigma = simProp.roughness_sigma), [wLftEdge, wRgtEdge, wBtmEdge, wTopEdge]))

    indx_elts = np.arange(len(EltCrack))

    _muPrime = muPrime[EltCrack] if isinstance(muPrime, np.ndarray) else muPrime
    FinDiffOprtr[indx_elts, indx_elts] = (-(wLftEdge ** 3 + wRgtEdge ** 3) / dx ** 2 - (
                                            wBtmEdge ** 3 + wTopEdge ** 3) / dy ** 2) / _muPrime
    FinDiffOprtr[indx_elts, neiInCrack[indx_elts, 0]] = wLftEdge ** 3 / dx ** 2 / _muPrime
    FinDiffOprtr[indx_elts, neiInCrack[indx_elts, 1]] = wRgtEdge ** 3 / dx ** 2 / _muPrime
    FinDiffOprtr[indx_elts, neiInCrack[indx_elts, 2]] = wBtmEdge ** 3 / dy ** 2 / _muPrime
    FinDiffOprtr[indx_elts, neiInCrack[indx_elts, 3]] = wTopEdge ** 3 / dy ** 2 / _muPrime
...
def Gravity_term(w, EltCrack, fluidProp, mesh, InCrack, simProp, cond):
    ...
    if simProp.gravity:
        if fluidProp.rheology == "Newtonian" and not fluidProp.turbulence:
            ...
            _muPrime = fluidProp.muPrime[EltCrack] if isinstance(fluidProp.muPrime, np.ndarray) else fluidProp.muPrime

            G[EltCrack] = fluidProp.density * 9.81 * (wTopEdge ** 3 - wBtmEdge ** 3) / mesh.hy / _muPrime
...
def FiniteDiff_operator_turbulent_implicit(w, pf, EltCrack, fluidProp, matProp, simProp, mesh, InCrack, vkm1, to_solve,
    ...
    for i in range(0, len(EltCrack)):
        ...
        _viscosity = fluidProp.viscosity[i] if isinstance(fluidProp.viscosity, np.ndarray) else fluidProp.viscosity
        # todo !!! Hack. zero velocity if the pressure gradient is zero or very small width
        if dpLft[i] < 1e-8 or wLftEdge[i] < 1e-10:
            vk[0, EltCrack[i]] = 0.0
        else:
            arg = (wLftEdge[i], _viscosity, fluidProp.density, dpLft[i], rough[i])
        ...
        if dpRgt[i] < 1e-8 or wRgtEdge[i] < 1e-10:
            ...
            arg = (wRgtEdge[i], _viscosity, fluidProp.density, dpRgt[i], rough[i])
            # check if bracket gives residuals with opposite signs
        ...
        if dpBtm[i] < 1e-8 or wBtmEdge[i] < 1e-10:
            ...
            arg = (wBtmEdge[i], _viscosity, fluidProp.density, dpBtm[i], rough[i])
            # check if bracket gives residuals with opposite signs
        ...
        if dpTop[i] < 1e-8 or wTopEdge[i] < 1e-10:
            ...
            arg = (wTopEdge[i], _viscosity, fluidProp.density, dpTop[i], rough[i])
            # check if bracket gives residuals with opposite signs
    ...
    _viscosity = fluidProp.viscosity[EltCrack] if isinstance(fluidProp.viscosity, np.ndarray) else fluidProp.viscosity
    ReLftEdge = 4 / 3 * fluidProp.density * wLftEdge * vk[0, EltCrack] / _viscosity
    ReRgtEdge = 4 / 3 * fluidProp.density * wRgtEdge * vk[1, EltCrack] / _viscosity
    ReBtmEdge = 4 / 3 * fluidProp.density * wBtmEdge * vk[2, EltCrack] / _viscosity
    ReTopEdge = 4 / 3 * fluidProp.density * wTopEdge * vk[3, EltCrack] / _viscosity
    ...
    indx_elts = np.arange(len(EltCrack))
    if simProp.solveSparse:
        FinDiffOprtr_csr = FinDiffOprtr.tocsr()
        n_rows = len(EltCrack)
        n_cols = len(to_solve) + len(active) + len(to_impose)
        from scipy.sparse import lil_matrix
        FD_compressed = lil_matrix((n_rows, n_cols), dtype=np.float64)
        FD_compressed[np.ix_(indx_elts, ch_indxs)] = FinDiffOprtr_csr[np.ix_(EltCrack, to_solve)]
        FD_compressed[np.ix_(indx_elts, act_indxs)] = FinDiffOprtr_csr[np.ix_(EltCrack, active)]
        FD_compressed[np.ix_(indx_elts, tip_indxs)] = FinDiffOprtr_csr[np.ix_(EltCrack, to_impose)]
        FD_compressed = FD_compressed.tocsr()
    else:
        FD_compressed = np.zeros((len(EltCrack), len(to_solve) + len(active) + len(to_impose)), dtype=np.float64)
...
def velocity(w, EltCrack, Mesh, InCrack, muPrime, C, sigma0):
    ...
    _muPrime = muPrime[EltCrack] if isinstance(muPrime, np.ndarray) else muPrime
    vel[0, EltCrack] = -((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 0]]) / 2) ** 2 / _muPrime * dpdxLft
    vel[1, EltCrack] = -((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 1]]) / 2) ** 2 / _muPrime * dpdxRgt
    vel[2, EltCrack] = -((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 2]]) / 2) ** 2 / _muPrime * dpdyBtm
    vel[3, EltCrack] = -((w[EltCrack] + w[Mesh.NeiElements[EltCrack, 3]]) / 2) ** 2 / _muPrime * dpdyTop
...
def calculate_fluid_flow_characteristics_laminar(w, pf, sigma0, Mesh, EltCrack, InCrack, muPrime, density):
    ...
    if np.min(muPrime) != 0: 
        ...
        _muPrime = muPrime[EltCrack] if isinstance(muPrime, np.ndarray) else muPrime
        fluid_flux = np.vstack((-wLftEdge ** 3 * dpLft / _muPrime, -wRgtEdge ** 3 * dpRgt / _muPrime))
        fluid_flux = np.vstack((fluid_flux, -wBtmEdge ** 3 * dpBtm / _muPrime))
        fluid_flux = np.vstack((fluid_flux, -wTopEdge ** 3 * dpTop / _muPrime))
        ...
        fluid_flux_components = np.vstack((-wLftEdge ** 3 * dp[0, EltCrack] / _muPrime, -wLftEdge ** 3 * dp[4, EltCrack] / _muPrime))
        #                                                      fx right edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wRgtEdge ** 3 * dp[1, EltCrack] / _muPrime))
        #                                                      fy right edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wRgtEdge ** 3 * dp[5, EltCrack] / _muPrime))
        #                                                      fx bottom edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wBtmEdge ** 3 * dp[6, EltCrack] / _muPrime))
        #                                                      fy bottom edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wBtmEdge ** 3 * dp[2, EltCrack] / _muPrime))
        #                                                      fx top edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wTopEdge ** 3 * dp[7, EltCrack] / _muPrime))
        #                                                      fy top edge
        fluid_flux_components = np.vstack((fluid_flux_components, -wTopEdge ** 3 * dp[3, EltCrack] / _muPrime))
        ...
        Rey_number = abs(4 / 3 * density * fluid_flux / np.mean(muPrime) * 12) #doesn't matter in a case of constant
