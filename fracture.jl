...
class Fracture:
    ...
    Attributes:
        ...
        EltChannel (ndarray):       -- list of cells currently in the channel r`````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````````egion
        ...
        muPrime (ndarray | np.ndarray):          -- local viscosity parameter
    ...
    def __init__(self, mesh, init_param, solid=None, fluid=None, injection=None, simulProp=None):
        ...
        if init_param.regime != 'static':
            ...
            _muPrime = np.max(fluid.muPrime) if isinstance(fluid.muPrime, np.ndarray) else fluid.muPrime
        ...
        # Sorting bu distances
        distances = np.array([(self.mesh.CenterCoor[x][0]**2 + self.mesh.CenterCoor[x][1]**2)**0.5 
                  for x in self.source])
        self.source = self.source[np.argsort(distances)]
        ...
        # local viscosity
        if fluid is not None and isinstance(fluid.muPrime, (int, float)):
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def remesh(self, factor, C, coarse_mesh, material_prop, fluid_prop, inj_prop, sim_prop, direction):
        ...
        if direction == None or direction == 'reduce':
            ...
            _muPrime = np.max(self.muPrime) if isinstance(self.muPrime, np.ndarray) else self.muPrime
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    def update_tip_regime(self, mat_prop, fluid_prop, timeStep):
        ...
        for i in moving:
            ...
            _muPrime = fluid_prop.muPrime[self.EltRibbon[i]] if isinstance(fluid_prop.muPrime, np.ndarray) else fluid_prop.muPrime
            wm = beta_m * (_muPrime * vel[i] / mat_prop.Eprime) ** (1/3)\
                 * (abs(self.sgndDist[self.EltRibbon[i]])) ** (2/3)
            wmtilde = beta_mtilde * (4 * _muPrime ** 2 * vel[i] * mat_prop.Cprime[self.EltRibbon[i]] ** 2
