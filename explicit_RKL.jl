...
def solve_width_pressure_RKL2(Eprime, GPU, n_threads, perf_node, *args):
    ...
    (to_solve, to_impose, wLastTS, pfLastTS, imposed_val, EltCrack, Mesh, dt, Q, C, muPrime, rho, InCrack, LeakOff, sigma0, turb, dgrain, gravity, active, wc_to_impose, wc, cf, neiInCrack) = args

    viscosity = muPrime / 12
    dt_CFL = 5 * np.mean(viscosity) * min(Mesh.hx, Mesh.hy) ** 3 / (Eprime * np.max(wLastTS) ** 3)
