...
export Integral_over_cell

function Integral_over_cell()
    ...
end
...
def Integral_over_cell(EltTip, alpha, l, mesh, function, frac=None, mat_prop=None, fluid_prop=None, Vel=None,
    ...
    while i < len(l):
        ...
        if fluid_prop is not None:
            _muPrime = fluid_prop.muPrime[EltTip][i] if isinstance(fluid_prop.muPrime, np.ndarray) else fluid_prop.muPrime
        else:
            _muPrime = None 
