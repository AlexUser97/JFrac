...
class MaterialProperties:
    ...
    Arguments:
        ...
        Eprime (float):                 -- plain strain modulus.c
    ...
    def __init__(self, Mesh, Eprime, toughness=0., Carters_coef=0., confining_stress=0., grain_size=0., K1c_func=None,
                 ...
                 Cij=None, free_surf=False, free_surf_depth=1.e300, TI_plane_angle=0., minimum_width=1e-10,
...
class FluidProperties:
    ...
    Attributes:
        viscosity (ndarray | np.ndarray):     -- Viscosity of the fluid (note its different from local viscosity, see
                                    fracture class for local viscosity).
        muPrime (float | np.ndarray):         -- 12 * viscosity (parallel plates viscosity factor).
        rheology (string):       -- string specifying rheology of the fluid. Possible options:
    ...
    def __init__(self, viscosity=None, density=1000., rheology="Newtonian", turbulence=False, compressibility=0,
        ...
        # elif isinstance(viscosity, np.ndarray):  # check if float or ndarray
            # NOTE: In the initial version of PyFrac viscosity was constant
...
class InjectionProperties:
    ...
    def __init__(self, rate, mesh, source_coordinates=None, source_loc_func=None, sink_loc_func=None,
                 ...
                 rate_delayed_inj_pt_func=None, delayed_second_injpoint_loc_func=None, check_cell_vertices=False):
        """
        The constructor of the InjectionProperties class.
        ATTENTION: check_cell_vertices is new fucntion.
        """
        self.check_cell_vertices = check_cell_vertices
        ...
        if rate_delayed_second_injpoint is not None and rate_delayed_inj_pt_func is None:
            if isinstance(rate_delayed_second_injpoint, np.ndarray):
                if rate_delayed_second_injpoint.shape[0] != 2:  # todo: check the condition
                    raise ValueError(
                        'Invalid injection rate of the delayed injection point. The list should have 2 rows (to specify time and'
                        ' corresponding injection rate) for each entry')
                elif rate_delayed_second_injpoint[0, 0] == 0:  # todo: check the condition
                    raise ValueError(
                        "The injection rate of the delayed injection point should start from a time >0 second i.e. rate[0, 0] should"
                        " be nonzero.")
        ...
        if delayed_second_injpoint_loc is not None:
            if isinstance(delayed_second_injpoint_loc, np.ndarray):
                ...
                self.delayed_second_injpoint_elem = [mesh.locate_element(self.delayed_second_injpoint_Coordinates[0],
                                                                         self.delayed_second_injpoint_Coordinates[1])]
            ...
            for i in range(mesh.NumberOfElts):
                if self.delayed_second_injpoint_loc_func(mesh.CenterCoor[i, 0], mesh.CenterCoor[i, 1], mesh.hx,
                                                         mesh.hy):
        ...
        if source_loc_func is None:
            ...
            self.sourceElem = []
            for i in range(mesh.NumberOfElts):
                if self.check_cell_vertices:
                    # Check all vertices
                    vertices = mesh.VertexCoor[mesh.Connectivity[i]]
                    inside = any(source_loc_func(v[0], v[1]) for v in vertices)
                else:
                    # Previous logic
                    inside = source_loc_func(mesh.CenterCoor[i, 0], mesh.CenterCoor[i, 1])
                if inside: self.sourceElem.append(i)
            
            # self.sourceElem.sort(key = lambda x: (mesh.CenterCoor[x][0]**2+mesh.CenterCoor[x][1]**2)**(1/2))
        if self.delayed_second_injpoint_elem is not None and not all(
                elem in self.sourceElem for elem in self.delayed_second_injpoint_elem):
            raise ValueError(
                "The delayed injection points elements are not contained in the list of all the injection elements")
    ...
    def remesh(self, new_mesh, old_mesh):
        ...
        if self.delayed_second_injpoint_loc_func != None:
            if self.sourceElem == None:
                ...
                self.delayed_second_injpoint_elem = []
            for i in range(new_mesh.NumberOfElts):
                if self.delayed_second_injpoint_loc_func(new_mesh.CenterCoor[i, 0], new_mesh.CenterCoor[i, 1],
...
class SimulationProperties:
    ...
    def __init__(self, address=None):
        ...
        self.roughness_model = simul_param.roughness_model
        self.roughness_sigma = simul_param.roughness_sigma

        # reference time
        self.refernce_time = simul_param.reference_time
    ...
    def set_simulation_name(self, simul_name):
        ...
        if simul_name is None:
            ...
            # print('simul_name name has to be specified')
        else:
            if not isinstance(simul_name, str):
                ...
                self.__simName = simul_name #corrected
    ...
    def set_mesh_extension_direction(self, direction):
        ...
        for i in direction:
            if i == 'vertical':
                ...
                raise ValueError(
                    'Invalid mesh extension definition Possible options: top, bottom, left, right, vertical'
                    'horizontal or all')
...
class RoughnessProperties:
    
    """
    This class stores the correction model function and it's parameters.
    """
